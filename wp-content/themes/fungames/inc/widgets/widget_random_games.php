...
if ( !class_exists('WP_Widget_MABP_Random_Games') ) {
  class WP_Widget_MABP_Random_Games extends WP_Widget {
    ...
    function __construct() {
      ...
      parent::__construct('MABP_Random_Games', 'MyArcade Game Scroller', $widget_ops);
    }
    ...
    function widget($args, $instance) {
      ...
      if ( !empty($blogcat) ) {
        $exclude = '&cat='.$category.$comma.'-'.$blogcat;
      }
      else {
        $exclude = '&cat=' .$category;
      }
        ...
        while( $games->have_posts() ) : $games->the_post();
          ...
          <li>
            <a href="<?php the_permalink() ?>" title="<?php the_title_attribute(); ?>">
              <?php myarcade_thumbnail( array( 'width' => 80, 'height' => 80, 'lazy_load' => false ) ); ?>
